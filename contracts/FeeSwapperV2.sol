...
/**
 * @title FeeSwapper V2 (DELTA)
 * @notice Swaps fee tokens to AERO with on-chain route optimization(-ish)
 * @dev 
 * FLOW:
 *   1. Splitter.collectFees() pushes non-AERO tokens here
 *   2. Keeper calls swap(tokens[]) with the token list
 ...
 *   6. All AERO returned to Splitter via processSwappedFees()
 *
 * ROUTE OPTIMIZATION:
...
contract FeeSwapperV2 is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Swap tokens to AERO using optimal routes, return to Splitter
     ...
     * @param tokens Tokens to swap (from Splitter push)
    ...
    function _findBestPool(address tokenA, address tokenB) internal view returns (PoolInfo memory best) {
        ...
        for (uint256 f; f < 2; ) {
            if (clFactories[f].code.length == 0) {
                unchecked { ++f; }
                continue;
            }
            for (uint256 i; i < 4; ) {
    ...
    /**
     * @notice Sweep stuck tokens (last hour of epoch only)
     * @param token Token to sweep
     * @param to Recipient
     */
    function sweepDust(address token, address to) external onlyOwner {
        if (splitter == address(0)) revert NoSplitter();
